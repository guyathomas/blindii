...
export default async (req, res) => {
  ...
  await manager.transaction(async (transactionManager) => {
    return await productOptionValueService
      ...
      .addProductOptionValues(validated.values);
  });
...
export class AdminPostProductsProductOptionsOptionValuesReq {
  @Type(() => AdminPostProductsProductOptionsOptionValue)
  @ValidateNested({ each: true })
  @IsArray()
  values: AdminPostProductsProductOptionsOptionValue[] = [];
}

export class AdminPostProductsProductOptionsOptionValue {
  @IsString()
  value: string;
